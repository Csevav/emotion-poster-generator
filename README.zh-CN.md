...
一个 AI skill，能把一个主题、一张照片、一句短文案，或一批 prompt，转换成安静、极简的 zine 风格情绪海报——高留白的做旧纸张、一个极小的象征性视觉锚点、微型排版，以及一个克制的强调色。
...
4. 按四段式结构编译最终的图像 prompt（画布与构图 / 主体与锚点 / 排版与强调色 / 情绪与负面约束),遵循一套严格的构图与配色规则。
5. 生成图像,通过质量校验（Quality Gate）,返回图像、最终 prompt,以及一段简短的解读说明。
